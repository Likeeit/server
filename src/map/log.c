...
void log_chat(e_log_chat_type type, int type_id, int src_charid, int src_accid, const char* map, int x, int y, const char* dst_charname, const char* message)
{
	...
	if( log_config.log_chat_woe_disable && agit_flag )
...
int log_remove_sell(int char_id, struct item *add_item, short selling_amount, short buying_amount) {
	if (selling_amount > buying_amount) {
		if( SQL_ERROR == Sql_Query(logmysql_handle, "UPDATE `%s` SET `amount` = '%d' WHERE `char_id`='%d' AND `nameid`='%d' AND `amount`='%d' AND `attribute`='%d' AND `refine`='%d' AND `card0`='%d' AND `card1`='%d' AND `card2`='%d' AND `card3`='%d' ORDER BY `id` ASC LIMIT 1",
		"sell_log", (selling_amount-buying_amount), char_id, add_item->nameid, selling_amount, add_item->attribute, add_item->refine, add_item->card[0], add_item->card[1], add_item->card[2], add_item->card[3]) )
		{
			Sql_ShowDebug(logmysql_handle);
			return 0;
		}
		...
		if( SQL_ERROR == Sql_Query(logmysql_handle, "DELETE FROM `%s` WHERE `char_id`='%d' AND `nameid`='%d' AND `amount`='%d' AND `attribute`='%d' AND `refine`='%d' AND `card0`='%d' AND `card1`='%d' AND `card2`='%d' AND `card3`='%d' ORDER BY `id` ASC LIMIT 1",
		"sell_log", char_id, add_item->nameid, selling_amount, add_item->attribute, add_item->refine, add_item->card[0], add_item->card[1], add_item->card[2], add_item->card[3]) )
		{
			Sql_ShowDebug(logmysql_handle);
			return 0;
		}
