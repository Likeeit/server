...
int duel_leave(const unsigned int did, struct map_session_data* sd)
{
	...
	// Debuff songs (improper remedy to rAthena bug#006523)
	...
	status_change_end(&sd->bl, SC_RICHMANKIM, INVALID_TIMER);
	status_change_end(&sd->bl, SC_ETERNALCHAOS, INVALID_TIMER);
	status_change_end(&sd->bl, SC_SIEGFRIED, INVALID_TIMER);
	status_change_end(&sd->bl, SC_INTOABYSS, INVALID_TIMER);
	return 0;
}
