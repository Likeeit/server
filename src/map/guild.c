...
bool guild_wardamage(struct map_session_data *sd)
{
	...
	if( !(battle_config.guild_wars && g) || sd->state.pvpmode || sd->duel_group > 0 || map_flag_noguildwar(sd->bl.m) || agit_flag )
		return false;
...
/*====================================================
 * alliance_send_message por: Zephyr (brAthena)
 *---------------------------------------------------*/
int alliance_send_message( struct map_session_data *sd, const char *mes, int len ) {
	int i;
	...
	g = guild_search( sd->status.guild_id );
	if( g ) {
		//clif_guild_message(g, sd->status.account_id, mes, len);
		guild_recv_message( g->guild_id, sd->status.account_id, mes, len );
		for( i = 0; i < guild_get_alliance_count( g, 0 ); i ++ ) {
			//intif_guild_message( g->alliance[i].guild_id, sd->status.account_id, mes, len );
			guild_recv_message( g->alliance[i].guild_id, sd->status.account_id, mes, len );
		}
		log_chat( LOG_CHAT_GUILD, sd->status.guild_id, sd->status.char_id, sd->status.account_id, mapindex_id2name( sd->mapindex ), sd->bl.x, sd->bl.y, NULL, mes );
	}
	return 0;
...
{	// Run All NPC_Event[OnAgitStart]
	...
	{
		if( (gc = guild_castle_search(c)) == NULL || gc->guild_id == 0 )
			continue;
