...
BUILDIN_FUNC(countitem)
{
	...
	for(i = 0; i < MAX_INVENTORY; i++)
	{
		...
		if (type > 0) {
			...
			if( battle_config.bg_reserved_char_id && char_id == battle_config.bg_reserved_char_id && type != 1)
				continue;
			if( battle_config.ancient_reserved_char_id && char_id == battle_config.ancient_reserved_char_id && !map[sd->bl.m].flag.ancient )
				continue;
			if( battle_config.woe_reserved_char_id && char_id == battle_config.woe_reserved_char_id && type != 2)
				continue;
		}
		else if( sd->status.inventory[i].card[0] == CARD0_CREATE)
		{
			char_id = MakeDWord(sd->status.inventory[i].card[2],sd->status.inventory[i].card[3]);
...
BUILDIN_FUNC(monster)
{
	...
	mob_once_spawn2(sd,m,x,y,str,class_,amount,event,hp_mod);
	return 0;
}
/*==========================================
...
BUILDIN_FUNC(mapannounce2)
{
	...
	TBL_PC* pl_sd;
	int type = pow(2, flag-1);
	int map_id = mapindex_name2id(map);
	if (!map_id)
		return -1;
	...
	for( pl_sd = (TBL_PC*)mapit_first(it); mapit_exists(it); pl_sd = (TBL_PC*)mapit_next(it) )
	{
		if( pl_sd->bl.m < 0 )
			continue; // not on any map
		if (pl_sd->mapindex != map_id) {
			continue; // not on the specified map
		}
		if( !(pc_readglobalreg(pl_sd, "announcements_on")&type) )
			continue; // ignoring this announcement
...
///
/// waitingroom "<title>",<limit>{,"<event>"{,<trigger>{,<zeny>{,<minlvl>{,<maxlvl>{,<upperBool>}}}}}};
BUILDIN_FUNC(waitingroom)
{
	...
	int upperBool = script_hasdata(st,9) ? 1 : 0;

	nd = (struct npc_data *)map_id2bl(st->oid);
	if( nd != NULL )
		chat_createnpcchat(nd, title, limit, pub, trigger, ev, zeny, minLvl, maxLvl, upperBool);
...
BUILDIN_FUNC(getmapflag)
{
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_NODISGUISE:			script_pushint(st,map[m].flag.nodisguise); break;
			...
			case MF_GVG_NOALLIANCE:		script_pushint(st,map[m].flag.gvg_noalliance); break;
		}
	}
...
BUILDIN_FUNC(setmapflag)
{
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_NODISGUISE:			map[m].flag.nodisguise = 1; break;
...
BUILDIN_FUNC(removemapflag)
{
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_NODISGUISE:			map[m].flag.nodisguise = 0; break;
			...
			case MF_GVG_NOALLIANCE:		map[m].flag.gvg_noalliance=0; break;
		}
	}
...
static int buildin_maprespawnguildid_sub_pc(struct map_session_data* sd, va_list ap)
{
	...
	while( 1 )
	{
		...
		if( g_id && (flag&4) && guild_isallied(g_id, sd->status.guild_id) && !map[m].flag.gvg_noalliance)
...
BUILDIN_FUNC(successremovecards)
{
	...
	{	// カードを取り除いたアイテム所得
		...
		item_tmp.serial = sd->status.inventory[i].serial,item_tmp.bound = sd->status.inventory[i].bound;
...
BUILDIN_FUNC(buyingstore)
{
	...
	if( map[sd->bl.m].flag.vending_cell != map_getcell(sd->bl.m,sd->bl.x,sd->bl.y,CELL_CHKNOVENDING) || map_count_oncell(sd->bl.m, sd->bl.x, sd->bl.y, BL_CHAR) > 1)
	{
...
BUILDIN_FUNC(tkrankreset)
{
	Sql* handle = mmysql_handle;
	struct map_session_data *sd;
	struct s_mapiterator* iter;
	iter = mapit_getallusers();
	// Store all fame into inactive_fame char variables for safe-keeping
	if(SQL_ERROR == Sql_Query(handle, "INSERT IGNORE INTO `global_reg_value` VALUES ((SELECT `char_id` FROM `char` WHERE (`fame` > 0 AND `class`='%d')), 'inactive_fame', (SELECT `fame` FROM `char` WHERE (`fame` > 0 AND `class`='%d')), 3, 0)", JOB_TAEKWON, JOB_TAEKWON))
		Sql_ShowDebug(handle);
	Sql_FreeResult(handle);
	// set all online TK fame to 0
	for( sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC*)mapit_next(iter) )
	{
	...
	// set TK fame to 0 in the database.
	if( SQL_ERROR == Sql_Query(handle, "UPDATE `char` SET `fame`=0 WHERE `class`='%d'", JOB_TAEKWON) )
	{
		Sql_ShowDebug(handle);
		return 1;
	}
	Sql_FreeResult(handle);
	mapit_free(iter);
	chrif_buildfamelist(); // re-build the fame list
	return 0;
}
...
BUILDIN_FUNC(restock)
{
	...
	int type = script_hasdata(st,4)? script_getnum(st,4) : 0;
	int i;
	int flag;
	int char_id;

	for (i=0; i < MAX_STORAGE; i++) {
		if (type > 0 && sd->status.storage.items[i].nameid == item_id) {
		...
		if (sd->status.storage.items[i].nameid == item_id) {
			if (sd->status.storage.items[i].card[0] == CARD0_CREATE) {
				char_id = MakeDWord(sd->status.storage.items[i].card[2],sd->status.storage.items[i].card[3]);
				if ((battle_config.bg_reserved_char_id && char_id == battle_config.bg_reserved_char_id) || (battle_config.woe_reserved_char_id && char_id == battle_config.woe_reserved_char_id))
					continue;
				else
					break;
			}
		...
		else if (i == (MAX_STORAGE-1))
			return 0;
		else
			continue;
	}

	if (amount > sd->status.storage.items[i].amount)
		amount = sd->status.storage.items[i].amount;

	if( (flag = pc_additem(sd,&sd->status.storage.items[i],amount,LOG_TYPE_STORAGE)) == 0 )
...
/*==================================================*
 * vending_add <item id>, <amount>, <price>{, <refine>{, <attribute>{, <card1>, <card2>, <card3>, <card4>{, "<name>"}}}};
 *--------------------------------------------------*/
BUILDIN_FUNC(vending_add)
{
	...
	vend->amount = script_getnum(st, 3);
	vend->value = script_getnum(st, 4);
	
	FETCH(5, vend->refine);
	FETCH(6, vend->attribute);

	for( i = 0; MAX_SLOTS > i; i++ )
		FETCH(7 + i, vend->card[i]);
...
BUILDIN_FUNC(vending_reset)
{
	...
	memset( sd->vend_buying, 0, sizeof (sd->vend_buying) );

	return 0;
}

BUILDIN_FUNC(skillup) {

	struct map_session_data* sd = script_rid2sd(st);
	int skill_id = script_getnum(st, 2);

	pc_skillup2(sd, skill_id);
	return 0;
...
struct script_function buildin_func[] = {
	// NPC interaction
	BUILDIN_DEF(skillup,"i"),
	BUILDIN_DEF(restock,"ii?"),
	BUILDIN_DEF(getguildmember,"i?"),
	BUILDIN_DEF(tkrankreset,""),
	BUILDIN_DEF(mes,"s"),
	BUILDIN_DEF(next,""),
	BUILDIN_DEF(close,""),
	BUILDIN_DEF(close2,""),
	...
	BUILDIN_DEF(announce2,"si?????"),
	BUILDIN_DEF(mapannounce2,"ssi?????"),
