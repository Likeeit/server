...
// Checks if player's current map has bonus exp event mapflag active
// Returns 1 if bexp, 2 if jexp, 3 if both and 0 if none.

int map_givesbonusexp(struct map_session_data *sd) {
	time_t timer;
	struct tm *t;
	int i;
	...
	if (!map[sd->bl.m].bexp_event.exp_bonus && !map[sd->bl.m].jexp_event.exp_bonus)
		return 0;

	if ((map[sd->bl.m].bexp_event.event_start_h == t->tm_hour && map[sd->bl.m].bexp_event.event_end_h > t->tm_hour && map[sd->bl.m].bexp_event.event_start_m <= t->tm_min)
		|| (map[sd->bl.m].bexp_event.event_start_h < t->tm_hour && map[sd->bl.m].bexp_event.event_end_h > t->tm_hour)
		|| (map[sd->bl.m].bexp_event.event_end_h == t->tm_hour && map[sd->bl.m].bexp_event.event_end_m >= t->tm_min))
		i+=1;

	if ((map[sd->bl.m].jexp_event.event_start_h == t->tm_hour && map[sd->bl.m].jexp_event.event_end_h > t->tm_hour && map[sd->bl.m].jexp_event.event_start_m <= t->tm_min)
		|| (map[sd->bl.m].jexp_event.event_start_h < t->tm_hour && map[sd->bl.m].jexp_event.event_end_h > t->tm_hour)
		|| (map[sd->bl.m].jexp_event.event_end_h == t->tm_hour && map[sd->bl.m].jexp_event.event_end_m >= t->tm_min))
		i+=2;
...
int map_quit(struct map_session_data *sd)
{
	...
	// save activity and afk points
	pc_setaccountreg(sd, "#afkpoints", pc_readaccountreg(sd,"#afkpoints")+sd->state.afkpoints);
	pc_setaccountreg(sd, "#activitypoints", pc_readaccountreg(sd,"#activitypoints")+sd->state.activitypoints);
	if (sd->votemute.time && ((int)time(NULL) - sd->votemute.time) <= 300) { // player is logging out while vote mute is in session.
		for (i=0; i < 128; i++) {
			if ( !mapreg_readregstr(reference_uid(add_str("$@votemute_names$"), i)) ) {
				mapreg_setregstr(reference_uid(add_str("$@votemute_names$"), i), sd->status.name);
				mapreg_setreg(reference_uid(add_str("$@votemute_counts"), i), sd->votemute.count);
				mapreg_setreg(reference_uid(add_str("$@votemute_times"), i), sd->votemute.time);
				break;
			}
		}
	}
...
void map_flags_init(void)
{
	...
	for( i = 0; i < map_num; i++ )
	{
		...
		// customizations
		map[i].flag.nobranch = 1; // Block DBs everywhere
	}
}
