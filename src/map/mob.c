...
bool mob_ksprotected (struct block_list *src, struct block_list *target)
{
	...
	do {
		...
		if( md->db->mexp || md->master_id || md->option.is_event || md->db->ffa || md->status.mode&MD_BOSS)
			return false; // MVP, Slaves and Event mobs ignores KS ,Czel: as well as miniboss
...
int mob_once_spawn2(struct map_session_data* sd, int m, short x, short y, const char* mobname, int class_, int amount, const char* event,unsigned int hp_mod)
{
	struct mob_data* md = NULL;
	int count, lv;
...
static void mob_item_drop(struct mob_data *md, struct item_drop_list *dlist, struct item_drop *ditem, int loot, int drop_rate, bool flag)
{
	...
	{ // Autoloot.
		type = 0;
		for (j=0; j < 5; j++) {
			if (sd->noloot[j] == ditem->item_data.nameid) {
				type = 1;
				break;
			}
		}
		switch(type) {
			case 1:
			if(  party_share_loot2(party_search(sd->status.party_id), sd, &ditem->item_data, sd->status.char_id) == 0 )
			{
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	if( sd )
	{
		...
		{ //TK_MISSION [Skotlex]
			if( ++sd->mission_count >= 100 && (temp = mob_get_random_id(0, 0xF, sd->status.base_level)) )
			{
				pc_addfame(sd, 1, 0);
