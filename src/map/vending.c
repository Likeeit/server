...
void vending_purchasereq_script(struct map_session_data* sd, struct npc_data* nd, int uid, const uint8* data, int count)
{
	...
	for( i = 0; count > i; i++ )
	{
		...
		if( index < 0 || index >= MAX_VENDING || sd->vend_buying[index].nameid == 0 )
			continue;
		
		data = itemdb_exists(sd->vend_buying[index].nameid);

		if( data == NULL )
			continue;

		zeny += (double)( sd->vend_buying[index].value * amount );
		...
		switch( pc_checkadditem(sd, sd->vend_buying[index].nameid, amount) )
		{
	...
	for( i = 0; count > i; i++ )
	{
		...
		add_item.nameid = sd->vend_buying[index].nameid;
		add_item.refine = sd->vend_buying[index].refine;
		add_item.identify = 1;
		add_item.attribute = sd->vend_buying[index].attribute;
		
		memcpy( add_item.card, sd->vend_buying[index].card, sizeof(sd->vend_buying[index].card) );

		if (log_remove_sell(sd->status.char_id, &add_item, sd->vend_buying[index].amount, amount) == 1) {
			pc_additem(sd, &add_item, amount, LOG_TYPE_NONE);
		}
	}
