...
//		return 0; //Cannot damage a bl not on a map, except when "charging" hp/sp
	...
	if( sc && sc->data[SC_KAIZEL] && !map_flag_gvg(target->m) && !map[target->m].flag.battleground && strcmp(map[target->m].name, "poring_w01"))
...
int status_change_start(struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4,int tick,int flag)
{
	...
	case SC_DONTFORGETME:
		...
		break;
	case SC_ONEHAND:
	  	//Removes the Aspd potion effect, as reported by Vicious. [Skotlex] but this is renewal!
	...
	switch(type)
	{
		...
		case SC_MARIONETTE:
		{
			...
			val3 = 0;
			val4 = 0;
			if (((TBL_PC*)pbl)->status.status_point >= 50)
				break;
