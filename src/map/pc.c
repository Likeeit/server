...
int pc_update_last_action(struct map_session_data *sd, int type)
{
	...
	idle_time = DIFF_TICK(last_tick,sd->idletime);
	if (sd->state.vending || sd->state.autotrade || !strcmp(map[sd->bl.m].name, "sec_pri"))
		idle_time = 0;

	if (idle_time > 30) { // consider as AFK time
		sd->state.afkpoints += ((idle_time+sd->state.afkpover)/battle_config.afk_point_gain);
		sd->state.afkpover = (idle_time+sd->state.afkpover)-(((idle_time+sd->state.afkpover)/battle_config.afk_point_gain)*battle_config.afk_point_gain);
	}
	else {
		sd->state.activitypoints += ((idle_time+sd->state.activitypover)/battle_config.activity_point_gain);
		sd->state.activitypover = (idle_time+sd->state.activitypover)-(((idle_time+sd->state.activitypover)/battle_config.activity_point_gain)*battle_config.activity_point_gain);
	}
...
void pc_addfame(struct map_session_data *sd,int count,short flag)
{
	...
	// Normal Rankings
	if (!sd->status.fame && pc_readglobalreg(sd, "inactive_fame"))
		sd->status.fame = pc_readglobalreg(sd, "inactive_fame");
	...
	switch(sd->class_&MAPID_UPPERMASK){
		...
		case MAPID_TAEKWON: // Taekwon
			pc_setglobalreg(sd, "inactive_fame", sd->status.fame);
...
bool pc_authok(struct map_session_data *sd, int login_id2, time_t expiration_time, int gmlevel, struct mmo_charstatus *st)
{
	...
	sd->vending_cantalk_tick = tick;
	sd->canjoinchn_tick = tick;
	sd->cansendmail_tick = tick;
	sd->keyboard_action_tick = tick;
	sd->mouse_action_tick = tick;
	sd->refreshdelay_tick = tick; // Czel
...
int pc_reg_received(struct map_session_data *sd)
{
	...
	sd->state.active = 1;
	sd->idletime = last_tick;
	sd->state.afkpoints = 0;
	sd->state.activitypoints = 0;
	...
	for (i=0; i < 128; i++) {
		if(mapreg_readregstr(reference_uid(add_str("$@votemute_names$"), i))
			&& !strncmp(sd->status.name, mapreg_readregstr(reference_uid(add_str("$@votemute_names$"), i)),strlen(sd->status.name)) ) {
			if (mapreg_readreg(reference_uid(add_str("$@votemute_counts"),i)) >= 50 ) {
				clif_manner_message(sd, 5);
				sd->status.manner -= 30;
				sc_start(&sd->bl,SC_NOCHAT,100,0,0);
			}
			else if ( (int)time(NULL) - (sd->votemute.time = mapreg_readreg(reference_uid(add_str("$@votemute_times"),i))) <= 300) {
				sd->votemute.count = mapreg_readreg(reference_uid(add_str("$@votemute_counts"),i));
			}
			mapreg_setregstr(reference_uid(add_str("$@votemute_names$"), i), "");
			mapreg_setreg(reference_uid(add_str("$@votemute_times"), i), 0);
			mapreg_setreg(reference_uid(add_str("$@votemute_counts"), i), 0);
			break;	
		}
	}
...
int pc_useitem(struct map_session_data *sd,int n)
{
	...
	{ // Do not allow use BG - Ancient Items on invalid maps
		...
		//if( battle_config.bg_reserved_char_id && char_id == battle_config.bg_reserved_char_id && (map[sd->bl.m].flag.pvp && !map[sd->bl.m].flag.battleground) ) //Disable use of BG items in PvP
		//	return 0;
...
static void pc_calcexp(struct map_session_data *sd, unsigned int *base_exp, unsigned int *job_exp, struct block_list *src)
{
	...
	switch(map_givesbonusexp(sd)) {
	case 1:
		ebonus+=map[sd->bl.m].bexp_event.exp_bonus;
		break;
	case 2:
		jbonus+=map[sd->bl.m].jexp_event.exp_bonus;
		break;
	case 3:
		ebonus+=map[sd->bl.m].bexp_event.exp_bonus;
		jbonus+=map[sd->bl.m].jexp_event.exp_bonus;
...
// Just a duplicate of the original function, with one small edit.
int pc_skillup2(struct map_session_data *sd,int skill_num)
{
	nullpo_ret(sd);
...
int pc_equipitem(struct map_session_data *sd,int n,int req_pos)
{
	...
	if(pos&(EQP_HEAD_LOW|EQP_COS_HEAD_LOW)) {
		...
		if(((pos&EQP_HEAD_LOW && (pc_checkequip(sd,EQP_COS_HEAD_LOW)) < 0) || pos&EQP_COS_HEAD_LOW) && (!pc_readglobalreg(sd,"disguise_look3") && !pc_readglobalreg(sd,"invi_d3")) || pc_readreg(sd,add_str("@disguiseoff")))
			clif_changelook(&sd->bl,LOOK_HEAD_BOTTOM,sd->status.head_bottom);
	}
	if(pos&(EQP_HEAD_TOP|EQP_COS_HEAD_TOP)) {
		...
		if(((pos&EQP_HEAD_TOP && (pc_checkequip(sd,EQP_COS_HEAD_TOP)) < 0) || pos&EQP_COS_HEAD_TOP) && (!pc_readglobalreg(sd,"disguise_look4") && !pc_readglobalreg(sd,"invi_d1")) || pc_readreg(sd,add_str("@disguiseoff")))
			clif_changelook(&sd->bl,LOOK_HEAD_TOP,sd->status.head_top);
	}
	...
	if(pos&(EQP_HEAD_MID|EQP_COS_HEAD_MID)) {
		...
		if(((pos&EQP_HEAD_MID && (pc_checkequip(sd,EQP_COS_HEAD_MID)) < 0) || pos&EQP_COS_HEAD_MID) && (!pc_readglobalreg(sd,"disguise_look5") && !pc_readglobalreg(sd,"invi_d2")) || pc_readreg(sd,add_str("@disguiseoff")))
			clif_changelook(&sd->bl,LOOK_HEAD_MID,sd->status.head_mid);
	}
	if(pos & EQP_SHOES)
		clif_changelook(&sd->bl,LOOK_SHOES,0);
...
int pc_unequipitem(struct map_session_data *sd,int n,int flag)
{
	...
	if(sd->status.inventory[n].equip & EQP_HEAD_LOW) {
		...
		if ((!pc_readglobalreg(sd,"disguise_look3") && !pc_readglobalreg(sd,"invi_d1")) || pc_readreg(sd,add_str("@disguiseoff")))
			clif_changelook(&sd->bl,LOOK_HEAD_BOTTOM,sd->status.head_bottom);
	}
	if(sd->status.inventory[n].equip & EQP_COS_HEAD_LOW) {
		...
		if ((!pc_readglobalreg(sd,"disguise_look3") && !pc_readglobalreg(sd,"invi_d3")) || pc_readreg(sd,add_str("@disguiseoff")))
			clif_changelook(&sd->bl,LOOK_HEAD_BOTTOM,sd->status.head_bottom);
	}
	if(sd->status.inventory[n].equip & EQP_HEAD_TOP) {
		...
		if ((!pc_readglobalreg(sd,"disguise_look4") && !pc_readglobalreg(sd,"invi_d1")) || pc_readreg(sd,add_str("@disguiseoff")))
			clif_changelook(&sd->bl,LOOK_HEAD_TOP,sd->status.head_top);
	}
	...
	if(sd->status.inventory[n].equip & EQP_HEAD_MID) {
		...
		if ((!pc_readglobalreg(sd,"disguise_look5") && !pc_readglobalreg(sd,"invi_d2")) || pc_readreg(sd,add_str("@disguiseoff")))
			clif_changelook(&sd->bl,LOOK_HEAD_MID,sd->status.head_mid);
	}
...
// Mimics dispell effect on player
int pc_dispell(struct block_list *bl) {
    int i;
	...
	for(i=0;i<SC_MAX;i++)
	{
		...
		case SC_CP_HELM:		case SC_COMBO:			case SC_STRFOOD:
		case SC_AGIFOOD:		case SC_VITFOOD:		case SC_INTFOOD:
		case SC_DEXFOOD:		case SC_LUKFOOD:		case SC_HITFOOD:
		case SC_FLEEFOOD:		case SC_BATKFOOD:		case SC_WATKFOOD:
		case SC_MATKFOOD:		case SC_DANCING:		case SC_GUILDAURA:
		case SC_AUTOBERSERK:	case SC_CARTBOOST:		case SC_MELTDOWN:
		case SC_SAFETYWALL:		case SC_SMA:			case SC_SPEEDUP0:
		case SC_NOCHAT:			case SC_ANKLE:			case SC_SPIDERWEB:
		case SC_JAILED:			case SC_ITEMBOOST:		case SC_EXPBOOST:
		case SC_LIFEINSURANCE:	case SC_BOSSMAPINFO:	case SC_PNEUMA:
		case SC_AUTOSPELL:		case SC_INCHITRATE:		case SC_INCATKRATE:
		case SC_NEN:			case SC_READYSTORM:		case SC_READYDOWN:
		case SC_READYTURN:		case SC_READYCOUNTER:	case SC_DODGE:
		case SC_WARM:			case SC_SPEEDUP1:		case SC_AUTOTRADE:
		case SC_CRITICALWOUND:	case SC_JEXPBOOST:		case SC_INVINCIBLE:
		case SC_INVINCIBLEOFF:	case SC_HELLPOWER:		case SC_MANU_ATK:
		case SC_MANU_DEF:		case SC_SPL_ATK:		case SC_SPL_DEF:
		case SC_MANU_MATK:		case SC_SPL_MATK:		case SC_RICHMANKIM:
		case SC_ETERNALCHAOS:	case SC_DRUMBATTLE:		case SC_NIBELUNGEN:
		case SC_ROKISWEIL:		case SC_INTOABYSS:		case SC_SIEGFRIED:
		case SC_FOOD_STR_CASH:	case SC_FOOD_AGI_CASH:	case SC_FOOD_VIT_CASH:
		case SC_FOOD_DEX_CASH:	case SC_FOOD_INT_CASH:	case SC_FOOD_LUK_CASH:
		case SC_EDP:
			continue;
		case SC_ASSUMPTIO:
			if( bl->type == BL_MOB )
