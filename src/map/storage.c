...
int storage_storageopen(struct map_session_data *sd)
{
	...
	{
		if (!(storage_flag = pc_readglobalreg(sd,"instance_storage")) ||
		!((strstr(map[sd->bl.m].name, "1@lhz") && storage_flag&1) ||
		(strstr(map[sd->bl.m].name, "ordeal_2-1") && storage_flag&2)
		// add future nostorage bypass maps here
		)) {
