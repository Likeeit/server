...
*/
	npc_event(sd,"CMDStorageCheck::OnBegin",0);
...
ACMD_FUNC(die)
{
	if(map[sd->bl.m].flag.battleground){ //Disabled @die in battleground maps
		return 0;
	}
...
ACMD_FUNC(go)
{
	...
	static const struct {
		...
		{ MAP_PRONTERA,    156, 187, "Prontera" }, // 0
		{ MAP_MORROC,      156,  93, "Morroc" }, // 1
		{ MAP_GEFFEN,      119,  59, "Geffen" }, // 2
		{ MAP_PAYON,       166, 227, "Payon" }, // 3
		{ MAP_ALBERTA,     192, 147, "Alberta" }, // 4
		{ MAP_IZLUDE,      128, 114, "Izlude" }, // 5
		{ MAP_ALDEBARAN,   141, 112, "Al De Baran" }, // 6
		{ MAP_LUTIE,       147, 134, "Lutie" }, // 7
		{ MAP_COMODO,      209, 143, "Comodo" }, // 8
		{ MAP_YUNO,        157, 168, "Yuno" }, // 9
		{ MAP_AMATSU,      114, 157, "Amatsu" }, // 10
		{ MAP_GONRYUN,     160, 120, "Gonryun" }, // 11
		{ MAP_UMBALA,       89, 157, "Umbala" }, // 12
		{ MAP_NIFLHEIM,     21, 153, "Niflheim" }, // 13
		{ MAP_LOUYANG,     217, 100, "Louyang" }, // 14
		{ MAP_NOVICE,       53, 111, "Novice Grounds" }, // 15
		{ MAP_JAIL,         23,  61, "Prison" }, // 16
		{ MAP_JAWAII,      204, 289, "Jawaii" }, // 17
		{ MAP_AYOTHAYA,    151, 117, "Ayothaya" }, // 18
		{ MAP_EINBROCH,     64, 200, "Einbroch" }, // 19
		{ MAP_LIGHTHALZEN, 158,  92, "Lighthalzen" }, // 20
		{ MAP_EINBECH,      80, 116, "Einbech" }, // 21
		{ MAP_HUGEL,        96, 145, "Hugel" }, // 22
		{ MAP_RACHEL,      130, 110, "Rachel" }, // 23
		{ MAP_VEINS,       216, 123, "Veins" }, // 24
		{ MAP_MOSCOVIA,    223, 184, "Moscovia" }, // 25
		{ MAP_MIDGARTS,	   217, 238, "Mid Camp" }, // 26=Midcamp
		{ MAP_SPLENDIDE,   195,  84, "Splendide" }, // 27=Splendide
		{ MAP_MANUK,       277, 223, "Manuk" }, // 28=Manuk
		{ MAP_ERISCASTLE,  198, 192, "Dicastes" }, // 29=Dicastes
		{ MAP_BRASILIS,    196, 218, "Brasilis" }, // 30=Brasilis
		{ MAP_VEND,         55,  64, "Vending" }, // 31=Vending
		{ MAP_BG,          155, 150, "Battle" }, // 32=BG		
		{ MAP_MOCRUINS,     67, 159, "Morocc Ruins" }, // 33=Moc Ruins
		{ "new_5-2",       100, 105, "Quest Zone" }, // 34=quest zone
		{ MAP_NPCZONE,      39,  40, "Npc Zone" }, // 35=npc zone
		{ "guild_vs3",       0,   0, "PvP Square" }, // 36=PvP
		{ "prontera",      155,	165, "King Of Emperium" }, // 37 = KoE
		{ "cmd_in02",	   211,	 96, "Casino" }, // 38 = Casino
		{ MAP_DEWATA,	   200,	226, "Dewata" }, // 39 = Dewata
		{ MAP_MALANGDO,	   181,	208, "Malangdo" }, // 40 = Malangdo
		{ MAP_MORA,	       114,	112, "Mora" }, // 41 = Mora
		{ "mosk_fild01",   103,	103, "Moscovia Field" }, // 42 = Moscovia Field
		{ "pay_arche",	    50,	134, "Payon Archers" }, // 43 = Payon Archers
	};
	...
	static const struct {
		...
		{ MAP_MIDGARTS,    "mid_camp" }, // 26=Midcamp
		{ MAP_SPLENDIDE,   "splendide" }, // 27=Splendide
		{ MAP_MANUK,       "manuk" }, // 28=Manuk
		{ MAP_ERISCASTLE,  "dicastes" }, // 29=Dicastes
		{ MAP_BRASILIS,    "brasilis" }, // 30=Brasilis
		{ MAP_VEND,        "vend" }, // 31=Vending
		{ MAP_BG,          "bg" }, // 32=BG		
		{ MAP_MOCRUINS,    "moc_ruins" }, // 33=Moc Ruins
		{ "new_5-2",       "questzone" }, // 34=quest zone
		{ MAP_NPCZONE,     "npczone" }, // 35=npc zone
		{ "guild_vs3",     "pvp" }, // 36=pvp
		{ "prontera",	   "koe" }, // koe
		{ "prontera",	   "king of emperium" }, // koe
		{ "cmd_in02",	   "casino" }, // Casino
		{ MAP_DEWATA,      "dewata" }, // 39 = Dewata
		{ MAP_MALANGDO,    "malangdo" }, // 40 = Malangdo
		{ MAP_MORA,        "mora" }, // 41 = Mora
		{ "mosk_fild01",   "moscovia field" }, // 42 = Moscovia Field
		{ "pay_arche",     "payon archers" }, // 43 = Payon Archers
	};
	...
	{ // input is a name
		...
		if (!strcmp(input, "koe") || !strcmp(input, "king")) town=37;
	...
	if(( data[town].mapname == "wglad") && (sd->status.base_level < 90) ){
		clif_displaymessage(fd,"You cannot participate in the arena until you are level 90 or above." );
		return -1;
	} 	
...
ACMD_FUNC(kickafk)
{
	...
	if (!map[pl_sd->bl.m].flag.kickafk) {
		clif_displaymessage(fd, "Player is not on a map that allows him to be kicked.");
		return -1;
	}
...
/*==========================================
 * Recall all characters with specific IP and optional map name parameter [Roy]
 *------------------------------------------*/
ACMD_FUNC(recallip)
{
	struct map_session_data* pl_sd;
	...
	if (!message || !*message || sscanf(message, "%s %s", ip_str, map_name) < 1) {
		clif_displaymessage(fd, "Invalid format. Please use: @recallip <ip address> <map name (optional)>.");
		return -1;
	}

	if (strlen(map_name) && !mapindex_name2id(map_name)) { // if map name provided, check if valid map
		clif_displaymessage(fd, "Invalid map name provided.");
		return -1;
	}
	...
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		if (strlen(map_name) && strcmp(map_name, map[pl_sd->bl.m].name))
			continue; // player isn't in the map specified
		if( pl_sd->status.account_id == sd->status.account_id )
			continue; // no point in recalling self
		if( pc_isGM(pl_sd) > pc_isGM(sd) )
			continue; // skip more powerful GMs
		...
		if (!sprintf(ip_str2, "%d.%d.%d.%d", CONVIP(session[pl_sd->fd]->client_addr)) || strcmp(ip_str, ip_str2))
			continue;
	...
	if (i > 0)
		...
		clif_displaymessage(fd,"No characters recalled.");
...
ACMD_FUNC(dance)
{
	...
	if (!message || !*message) {
		numdance = rand() % 9 + 1;//randomize if message not placed
	}
	else {
		numdance = atoi(message);
	}
	if ( numdance == 1 ) {
...
{
	unsigned int tick = gettick();
	if(DIFF_TICK(sd->refreshdelay_tick,tick)>0)
	{
		return 0;
	}
	nullpo_retr(-1, sd);
	clif_refresh(sd);
	sd->refreshdelay_tick = tick+battle_config.refresh_delay;
	return 0;
}
 ...
 *------------------------------------------*/
static int whosell_comp_price_ascending(const void *_i1, const void *_i2)
{
	struct whosell_data *i1 = (struct whosell_data *)_i1;
	struct whosell_data *i2 = (struct whosell_data *)_i2;

	if (i1->value < i2->value) return -1;
	else if (i1->value == i2->value) return 0;
	else return 1;
}
static int whosell_comp_price_descending(const void *_i1, const void *_i2)
{
	struct whosell_data *i1 = (struct whosell_data *)_i1;
	struct whosell_data *i2 = (struct whosell_data *)_i2;
...
ACMD_FUNC(whosell)
{
	...
	for( i = 0; i < count; i++ )
	{
		...
		{
			sprintf(output, "[%d] The best price for '%s' is %u from '%s' in %s <%d,%d>. Max %u. Found in %d shops, %d for sale.", item_array[i]->nameid, item_array[i]->jname, MinPrice[i], b_sd[i]->status.name, map[b_sd[i]->bl.m].name, b_sd[i]->bl.x, b_sd[i]->bl.y, MaxPrice[i], total[i], amount[i]);
...
ACMD_FUNC(whosellenchant)
{
	struct map_session_data *pl_sd, *b_sd[MAX_SEARCH];
	struct s_mapiterator* iter;

	struct item_data *item_array[MAX_SEARCH];
	int total = 0, amount[MAX_SEARCH];
	unsigned int MinPrice[MAX_SEARCH], MaxPrice[MAX_SEARCH];
	char output[256];
	char sort[23];
	char item_name[100];
	int i, j, count = 1,enchantid=0;
	nullpo_retr(-1, sd);
	...
	if (!message || !*message || (
		sscanf(message, "\"%99[^\"]\" %d %23s", item_name, &enchantid, sort) < 2 &&
		sscanf(message, "%99s %d %23s", item_name,&enchantid, sort) < 2
	)) {
		clif_displaymessage(fd, "Please, enter Item name or its ID (usage: @whosellenchant <item name or ID> <enchant> {d/a}).");
		return -1;
	}
	if( (item_array[0] = itemdb_exists(atoi(item_name))) == NULL )
	...
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		if (!pl_sd->state.vending) continue;
		for( i = 0; i < pl_sd->vend_num; i++ )
		{ // Searching in the Vending List
			for( j = 0; j < count; j++ )
			{ // Compares with each search result
				if( pl_sd->status.cart[pl_sd->vending[i].index].nameid != item_array[j]->nameid )
					continue;
				if( pl_sd->status.cart[pl_sd->vending[i].index].card[3] != enchantid)
					continue;
				amount[j] += pl_sd->vending[i].amount;
				total++;
				...
				sd->whoselllist[total].value = pl_sd->vending[i].value;
				sd->whoselllist[total].amount = pl_sd->vending[i].amount;
				sd->whoselllist[total].id = pl_sd->bl.id;
			}
		}
	}
	mapit_free(iter);
	if(strcmp(sort,"d") == 0)
		qsort(sd->whoselllist, total+1 , sizeof(struct whosell_data), whosell_comp_price_descending);
	else
		qsort(sd->whoselllist, total+1 , sizeof(struct whosell_data), whosell_comp_price_ascending);
	if(total){
		if(strcmp(sort,"d") == 0)
			clif_displaymessage(sd->fd, "Sorted Descending");
		else
			clif_displaymessage(sd->fd, "Sorted Asscending");	
		for( i = 0; i <= total; i++ )
		{
			struct map_session_data *sd_sorted;
			sd_sorted = map_id2sd(sd->whoselllist[i].id);
			if(!sd_sorted) continue;
			sprintf(output, "[%d] Price:%d - Name:%s - Amount:%d - Shop:%s - Loc:%s,%d,%d",
			item_array[0]->nameid,sd->whoselllist[i].value,itemdb_jname(item_array[0]->nameid),sd->whoselllist[i].amount,sd_sorted->message,map[sd_sorted->bl.m].name,sd_sorted->bl.x,sd_sorted->bl.y);
			clif_displaymessage(sd->fd, output);
		}
		memset(sd->whoselllist, 0, sizeof(struct whosell_data));
		return 0;
	}
	sprintf(output, "[%d] '%s' Item not found.", item_array[0]->nameid, item_array[0]->jname);		
	clif_displaymessage(sd->fd, output);
	return 0;
}

ACMD_FUNC(whosellrefine)
{
	struct map_session_data *pl_sd, *b_sd[MAX_SEARCH];
	struct s_mapiterator* iter;
	...
	if (!message || !*message || (
		sscanf(message, "\"%99[^\"]\" %d %23s", item_name, &refine, sort) < 2 &&
		sscanf(message, "%99s %d %23s", item_name,&refine, sort) < 2
	)) {
		clif_displaymessage(fd, "Please, enter Item name or its ID (usage: @whosellrefine <item name or ID> <refine> {d/a}).");
		return -1;
	}
	if( (item_array[0] = itemdb_exists(atoi(item_name))) == NULL )
		count = itemdb_searchname_array(item_array, MAX_SEARCH, item_name);
	...
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		...
		{ // Searching in the Vending List
			...
			{ // Compares with each search result
				if( pl_sd->status.cart[pl_sd->vending[i].index].nameid != item_array[j]->nameid )
					continue;
				if( pl_sd->status.cart[pl_sd->vending[i].index].refine != refine)
					continue;
				amount[j] += pl_sd->vending[i].amount;
				total++;
...
ACMD_FUNC(request)
{
	...
	mes_out = &mes[a];
	memset(mes_out, '\0', 1);
	if( SQL_ERROR == Sql_Query(handle, "INSERT INTO `log_request` (`char_id`, `char_name`,`message`) VALUES ('%d', '%s', '%s')", sd->status.char_id, sd->status.name, mes))
...
ACMD_FUNC(reportafk)
{
	...
	if (!strcmp(map[sd->bl.m].name, "ordeal_1-1")) {
		if (!message || !*message)
			clif_displaymessage(fd, "Please, enter the character name (usage: @reportafk <name>).");
		else if( (pl_sd = map_nick2sd((char *)message)) == NULL )
			clif_displaymessage(fd, msg_txt(3)); // Character not found.
		else if( strcmp(map[pl_sd->bl.m].name, "ordeal_1-1") )
			clif_displaymessage(fd, "Player is not in Devil Square.");
		else if( sd == pl_sd )
			clif_displaymessage(fd, "You cannot kick yourself.");
		else if ((idle_time=DIFF_TICK(last_tick,pl_sd->idletime)) < 120) {
			sprintf(output, "Idle time: %ds", idle_time);
			clif_displaymessage(fd,output);			
		}	
		else {
			pc_setpos(pl_sd,pl_sd->status.save_point.map,pl_sd->status.save_point.x,pl_sd->status.save_point.y,CLR_TELEPORT);
			clif_displaymessage(pl_sd->fd,"You have been kicked out of Devil Square for being inactive.");
			return 0;
		}
	}
	else {
		if( !sd->bg_id )
			clif_displaymessage(fd, "This command is reserved for Battleground/Devil Square Only.");
...
ACMD_FUNC(alliance) {
	if(!message || !*message)
		return 0;
	clif_alliance_chat(sd,message);
	return 0;
}

ACMD_FUNC(jumpwhosell)
{
	char item_name[100];
	...
	struct map_session_data* pl_sd;
	struct s_mapiterator* iter;
	unsigned int MinPrice = battle_config.vending_max_value;
	struct item_data *item_data;

	nullpo_retr(-1, sd);

	if(strcmp(map[sd->bl.m].name, "wvend")) {
		clif_displaymessage(fd, "This command is only usable on vending maps.");
		return -1;
	}
	...
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		if( pl_sd->state.vending ) //check if player is vending
		{
			...
			for (j = 0; j < pl_sd->vend_num; j++) {
				if(pl_sd->status.cart[pl_sd->vending[j].index].nameid == item_id && pl_sd->vending[j].value < MinPrice) {
...
ACMD_FUNC(votemute) {
	struct map_session_data *pl_sd;
	char announce[128];
	int manner = 30; // how long player is muted for (minutes)
	int i;	
	uint32 ip;
	char ip_addr[64];
	int c = 0;
	...
	ip = session[sd->fd]->client_addr;
	sprintf(ip_addr,"%d.%d.%d.%d",CONVIP(ip));

	for (i=0; i < 128; i++) {
		if ( sprintf(ipbuff,"%s",mapreg_readregstr(reference_uid(add_str("$votemute_ip$"), i))) > 7 )
			c++;
		if ( !strcmp(ip_addr, ipbuff) ) {
			blocked = true;
		}
	}

	if (blocked == true) {
		clif_displaymessage(fd, "This command can only be used once every 24 hours per IP.");
		return -1;	
	}

	if (agit_flag) {
		clif_displaymessage(fd, "This command is disabled during WoE.");
		return -1;
	}
	...
	if (c >= 128) {
		clif_displaymessage(fd, "Unexpected error has occured. Please notify a GM."); // vote array is full
		return -1;
	}
	...
	{
		for (i=0; i < 128; i++) {
			if ( mapreg_readregstr(reference_uid(add_str("$@votemute_names$"), i)) && !strcmp(mapreg_readregstr(reference_uid(add_str("$@votemute_names$"), i)), message) &&
				((int)time(NULL) - mapreg_readreg(reference_uid(add_str("$@votemute_times"), i))) <= 300 && mapreg_readreg(reference_uid(add_str("$@votemute_counts"), i)) < 50 ) 
			{
				tempstore = mapreg_readreg(reference_uid(add_str("$@votemute_counts"), i)) + 1;
				mapreg_setreg(reference_uid(add_str("$@votemute_counts"), i), tempstore);
				snprintf(dispmsg, sizeof(dispmsg), "Vote successful. %d votes left until player will be muted.", (50-tempstore));
				clif_displaymessage(fd, dispmsg);
				if (tempstore >= 50) {
					snprintf(announce, 128, "50 people have voted for %s to be muted! %s has been muted for 30 minutes.", message, message);
					clif_specialannounce(sd, announce, 5, 1, 0, 0, 0, 0, 0);
				}
				return 0;
			}
			if (i >= 127) {
	...
	if (sd == pl_sd) {
		clif_displaymessage(fd, "You can't vote to mute yourself.");
		return -1;
	}

	if (pl_sd->status.manner < 0) {
		clif_displaymessage(fd, "Player is already muted.");
		return -1;
	}

	if (!pl_sd->votemute.time || (pl_sd->votemute.time && ((int)time(NULL) - pl_sd->votemute.time) > 300)) { // Begin new voting session
		if (pc_isGM(sd) < 1) {
			clif_displaymessage(fd, "Only council members and GMs can start votes."); // Character not found.
			return -1;
		}
		snprintf(announce, 128, "%s has been voted to be muted, type \"@votemute %s\" if you agree! %s will be muted if there are 50 votes by 5 mins.", message, message,((pl_sd->status.sex) ? "He":"She"));
		clif_specialannounce(sd, announce, 5, 1, 0, 0, 0, 0, 0);
		pl_sd->votemute.time = last_tick;
		pl_sd->votemute.count = 1;
		snprintf(dispmsg, sizeof(dispmsg), "Vote successful. %d votes left until player will be muted.", (50-pl_sd->votemute.count));
		clif_displaymessage(fd, dispmsg);
	}
	else {
		pl_sd->votemute.count++;
		snprintf(dispmsg, sizeof(dispmsg), "Vote successful. %d votes left until player will be muted.", (50-pl_sd->votemute.count));
		clif_displaymessage(fd, dispmsg);
		if (pl_sd->votemute.count >= 50) { // mute the target player
			snprintf(announce, 128, "50 people have voted for %s to be muted! %s has been muted for 30 minutes.", message, message);
			clif_specialannounce(sd, announce, 5, 1, 0, 0, 0, 0, 0);
			clif_manner_message(pl_sd, 5);
			pl_sd->status.manner -= manner;
	...
	mapreg_setregstr(reference_uid(add_str("$votemute_ip$"), c), ip_addr); // store vote ip
	mapreg_setreg(reference_uid(add_str("$votemute_time"), c), (int)time(NULL)); // store vote time
	return 0;
...
ACMD_FUNC(noloot) {
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Invalid use of command. Usage: @noloot <slot 1-5> <item id/name/clear> | @noloot list");
		return -1;
	}

	if (!strcmp(message, "list")) {
		clif_displaymessage(fd, "-- No Loot Item List --");
		for (i=0; i < 5; i++) {
			snprintf(output, 100, "Slot %d - %s", (i+1), ((item_data = itemdb_exists(sd->noloot[i])) ? item_data->jname : "Empty"));
			clif_displaymessage(fd, output);
		}
		return 0;
	}
	if (sscanf(message, "%d %99[^\n]", &i, item_name) < 2) {
		clif_displaymessage(fd, "Invalid use of command. Usage: @noloot <slot 1-5> <item id/name> | @noloot list");
		return -1;
	}
	else if(i > 5 || i < 1) {
		clif_displaymessage(fd, "Invalid slot number. Choose a number between 1 and 5 (inclusive).");
		return -1;
	}
	else if (!strcmp(item_name, "clear")) {
		sd->noloot[i-1] = NULL;
		snprintf(output, 100, "Slot %d has been cleared.", i);
		clif_displaymessage(fd, output);
		return 0;
	}
	else if ((item_data = itemdb_searchname(item_name)) == NULL && (item_data = itemdb_exists(atoi(item_name))) == NULL) {
		clif_displaymessage(fd, "Invalid Item ID/Name");
		return -1;		
	}
	else {
		sd->noloot[i-1] = item_data->nameid;
		clif_displaymessage(fd, "-- No Loot Item List --");
		for (i=0; i < 5; i++) {
			snprintf(output, 100, "Slot %d - %s", (i+1), ((item_data = itemdb_exists(sd->noloot[i])) ? item_data->jname : "Empty"));
			clif_displaymessage(fd, output);
...
ACMD_FUNC(votekick)
{
	...
	if (!sd->votekick.mytime) sd->votekick.mytime = 0;
	
	if (!strcmp(map[sd->bl.m].name, "ordeal_1-1")) {
		if (!message || !*message)
			clif_displaymessage(fd, "Please, enter the character name (usage: @votekick <name>).");
		else if( (pl_sd = map_nick2sd((char *)message)) == NULL )
			clif_displaymessage(fd, msg_txt(3)); // Character not found.
		else if( strcmp(map[pl_sd->bl.m].name, "ordeal_1-1") )
			clif_displaymessage(fd, "Player is not in Devil Square.");
			...
			clif_displaymessage(fd, "You cannot kick yourself.");
		else if ((remaining_time = 1800-((int)time(NULL) - sd->votekick.mytime)) > 0) {
			sprintf(my_output, "Cannot kick yet. %d second(s) left.", remaining_time);
			clif_displaymessage(fd, my_output);
		}	
		else {
			if (!pl_sd->votekick.time) pl_sd->votekick.time = 0;
			sd->votekick.mytime = (int)time(NULL);
			if ((int)time(NULL) - pl_sd->votekick.time > 60) {
				pl_sd->votekick.time = (int)time(NULL);
				pl_sd->votekick.count = 1;
				sprintf(atcmd_output, "Vote kick for \"%s\" has started. 14 more votes until he is kicked from Devil Square!", pl_sd->status.name);
				clif_specialannounce(sd, atcmd_output, 5, 1, 0, 0, 0, 0, 0);
				return 0;
			}
			pl_sd->votekick.count++;
			pl_sd->votekick.time = (int)time(NULL);
			if (pl_sd->votekick.count >= 15) {
				sprintf(atcmd_output, "%s has been kicked from Devil Square!", pl_sd->status.name);
			}
			else {
				sprintf(atcmd_output, "%d more votes until %s is kicked from Devil Square!", 15-pl_sd->votekick.count, pl_sd->status.name);
			}
			clif_specialannounce(sd, atcmd_output, 5, 1, 0, 0, 0, 0, 0);
			if (pl_sd->votekick.count >= 15) {
				pc_setpos(pl_sd,pl_sd->status.save_point.map,pl_sd->status.save_point.x,pl_sd->status.save_point.y,CLR_TELEPORT);
				clif_displaymessage(pl_sd->fd,"You have been vote kicked out of Devil Square.");
			}
			return 0;
		}
	}
		...
		{ // Everytest OK!
			...
			if ((int)time(NULL) - pl_sd->votekick.time > 60) {
				pl_sd->votekick.time = (int)time(NULL);
				pl_sd->votekick.count = 1;
				sprintf(atcmd_output, "Vote kick for \"%s\" has started. 14 more votes until he is kicked from Battlegrounds!", pl_sd->status.name);
				clif_specialannounce(sd, atcmd_output, 5, 1, bg->color, 0x190, 20, 0, 0);
				return 0;
			}
			pl_sd->votekick.count++;
			pl_sd->votekick.time = (int)time(NULL);
			if (pl_sd->votekick.count >= 15) {
				bg_team_leave(pl_sd,2);
				clif_displaymessage(pl_sd->fd, "You have been vote kicked out of Battlegrounds.");
				pc_setpos(pl_sd,pl_sd->status.save_point.map,pl_sd->status.save_point.x,pl_sd->status.save_point.y,3);

				sprintf(atcmd_output, "%s has been vote kicked out of Battlegrounds.", pl_sd->status.name);
				clif_specialannounce(sd, atcmd_output, 5, 1, bg->color, 0x190, 20, 0, 0);
				return 0;
			}
			else {
...
ACMD_FUNC(voteleader)
{
	...
	if (!bg->voteleader.time) bg->voteleader.time = 0;
	if (!bg->voteleader.char_id) bg->voteleader.char_id = 0;
	if( !sd->bg_id || (bg = bg_team_search(sd->bg_id)) == NULL)
		clif_displaymessage(fd, "This command is reserved for Battlegrounds Only.");
	else if( !message || !*message )
		clif_displaymessage(fd, "Please, enter the character name (usage: @voteleader <name>).");
	else if( (pl_sd = map_nick2sd((char *)message)) == NULL )
		clif_displaymessage(fd, msg_txt(3)); // Character not found.
	else if( sd->bg_id != pl_sd->bg_id ) {
		clif_displaymessage(fd, "Destination Player is not in your Team.");
	}
	else if( pl_sd->status.char_id == bg->leader_char_id )
		clif_displaymessage(fd, "Destination Player is already the leader.");
	else if ((int)time(NULL) < bg->voteleader.time + 120 && pl_sd->status.char_id != bg->voteleader.char_id)
		clif_displaymessage(fd, "A leader voting session is already taking place, you cannot vote for a new leader.");
	else
	{
		int i;
		int c;
		for (i=0; i < MAX_BG_MEMBERS; i++) {
			if(!bg->voteleader.voted[i] && c != NULL) {
				c = i;
			}
			if (bg->voteleader.voted[i] && bg->voteleader.voted[i] == sd->status.char_id) {
				clif_displaymessage(fd, "You can only vote once per session.");
				return -1;
			}
		}
		bg->voteleader.voted[c] = sd->status.char_id;
		if ((int)time(NULL) - bg->voteleader.time > 120) {
			bg->voteleader.count_req = (int)ceil(bg->count*0.4);
			bg->voteleader.time = (int)time(NULL);
			bg->voteleader.count = 1;
			bg->voteleader.char_id = pl_sd->status.char_id;
			if (bg->voteleader.count >= bg->voteleader.count_req) {
				bg->leader_char_id = pl_sd->status.char_id;
				pl_sd->bmaster_flag = bg;
				bg->voteleader.time = 0;
				for (i=0; i < MAX_BG_MEMBERS; i++) {
					bg->voteleader.voted[i] = 0;
					if(bg->members[i].sd && bg->members[i].sd->bmaster_flag && bg->members[i].sd->status.char_id != bg->leader_char_id) {
						bg->members[i].sd->bmaster_flag = NULL;
						clif_charnameupdate(bg->members[i].sd);
					}
				}
				clif_charnameupdate(pl_sd);
				sprintf(atcmd_output, "%s has been voted as the new leader of the team.", pl_sd->status.name);
			}
			else {
				sprintf(atcmd_output, "Voting to assign \"%s\" as new leader has started. Use \"@voteleader %s\"", message, message);
			}
			for( i=0; i < MAX_BG_MEMBERS; i++ )
			{
				if( !bg->members[i].sd)
				...
				clif_displaymessage(bg->members[i].sd->fd, atcmd_output);
			}
			return 0;
		}
		bg->voteleader.count++;
		if (bg->voteleader.count >= bg->voteleader.count_req) {
			...
			sprintf(atcmd_output, "%d more votes until %s is elected as the new leader. %d seconds left.", bg->voteleader.count_req-bg->voteleader.count, message,((int)time(NULL))-bg->voteleader.time);
			for( i=0; i < MAX_BG_MEMBERS; i++ )
			{
				if( !bg->members[i].sd)
					continue;
...
AtCommandInfo atcommand_info[] = {
	...
	{ "dance",              0,60,   0,     atcommand_dance }, //Czel's custom commands goes here
	{ "alliance",               0,60,   0,     atcommand_alliance },
	{ "whosellrefine",	0,60,	0,	atcommand_whosellrefine },
	{ "whosellenchant",	0,60,	0,	atcommand_whosellenchant },
	{ "jumpwhosell",	0,60,	0,	atcommand_jumpwhosell },
	{ "whobuy",	0,60,	0,	atcommand_whobuy },
	{ "show",	0,60,	0,	atcommand_show },
	{ "kickafk",	5,60,	0,	atcommand_kickafk },
	{ "votemute",	0,60,	0,	atcommand_votemute },
	{ "noloot",              0,40,   0,     atcommand_noloot },
	{ "votekick",              0,40,   0,     atcommand_votekick },
	{ "recallip",              0,40,   0,     atcommand_recallip },
	{ "voteleader",              0,40,   0,     atcommand_voteleader },
};
