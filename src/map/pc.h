...
struct map_session_data {
	...
	struct {
		...
		unsigned int afkpoints;
		unsigned int activitypoints;
		unsigned int afkpover;
		unsigned int activitypover;
		...
		unsigned int npc_vending; // Player has a vending open from a script
		...
		unsigned int snovice_call_flag : 3; //Novice Chant
	} state;
	struct {
	...
	struct whosell_data whoselllist[MAX_INVENTORY];
	...
	unsigned int vending_cantalk_tick; // [Channel Flood Protection - Vending Channel]
	unsigned int canjoinchn_tick;
	unsigned int cansendmail_tick; // [Mail System Flood Protection]
	unsigned int ks_floodprotect_tick; // [Kill Steal Protection]
	unsigned int refreshdelay_tick;
	struct {
		int nameid;
		unsigned int tick;
	...
	struct npc_item_vend vend_buying[MAX_VENDING];
...
int pc_skillup2(struct map_session_data*,int);
