	...
	if (p && p->party.item&2 && (first_charid || !(battle_config.party_share_type&1)))
	{
		...
		{	//Round Robin
			...
			do {
				...
				noloot = false;
				for (j=0; j < 5; j++) {
					if (psd->noloot[j] == item_data->nameid) {
						noloot = true;
						break;
					}
				}
		...
		{	//Random pick
			...
			while (count > 0) { //Pick a random member.
				...
				noloot = false;
				for (j=0; j < 5; j++) {
					if (psd[i]->noloot[j] == item_data->nameid) {
						noloot = true;
						break;
					}
				}
				if (noloot == true || pc_additem(psd[i],item_data,item_data->amount,LOG_TYPE_PICKDROP_PLAYER))
...
// variant of party_share_loot, except person picking up the item cannot loot
int party_share_loot2(struct party_data* p, struct map_session_data* sd, struct item* item_data, int first_charid)
{
	TBL_PC* target = NULL;
	int i, j;
	bool noloot = false;
