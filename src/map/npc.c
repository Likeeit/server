...
static const char* npc_parse_mapflag(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	else if (!strcmpi(w3,"bg_topscore"))
	{
	...
	else if (!strcmpi(w3,"nodisguise"))
		map[m].flag.nodisguise=state;
		...
		if (sscanf(w4, "%d,%d,%d,%d,%d", &bstart_h, &bstart_m, &bend_h, &bend_m, &bbonus_exp) == 5) { // mapflag set acceptably
			map[m].flag.bexp_event = 1;
			map[m].bexp_event.event_start_h = bstart_h;
			map[m].bexp_event.event_start_m = bstart_m ;
			map[m].bexp_event.event_end_h = bend_h;
			map[m].bexp_event.event_end_m = bend_m;
			if (bbonus_exp < 0)
				bbonus_exp = 0;
			map[m].bexp_event.exp_bonus = bbonus_exp;
		}


	}
	else if (!strcmpi(w3,"jexp_event")) {
		int jstart_h;
		int jstart_m;
		int jend_h;
		int jend_m;
		...
		if (sscanf(w4, "%d,%d,%d,%d,%d", &jstart_h, &jstart_m, &jend_h, &jend_m, &jbonus_exp) == 5) { // mapflag set acceptably
			map[m].flag.jexp_event = 1;
			map[m].jexp_event.event_start_h = jstart_h;
			map[m].jexp_event.event_start_m = jstart_m ;
			map[m].jexp_event.event_end_h = jend_h;
			map[m].jexp_event.event_end_m = jend_m;
			if (jbonus_exp < 0)
				jbonus_exp = 0;
			map[m].jexp_event.exp_bonus = jbonus_exp;
		}
	}
	else if (!strcmpi(w3,"kickafk"))
		map[m].flag.kickafk=state;
	else
